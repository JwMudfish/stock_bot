...
class collector_api():
    ...
    def variable_setting(self):
        ...
        self.dc=daily_crawler(self.open_api.cf.real_db_name, self.open_api.cf.real_daily_craw_db_name,self.open_api.cf.real_daily_buy_list_db_name)
    ...
    def get_code_list(self):
        ...
        df_stock_all_temp = {'id': [], 'code': [], 'code_name': [], 'check_item': [],'check_daily_crawler': [], 'check_min_crawler': []}
        self.df_stock_all = DataFrame(df_stock_all_temp, columns=['code', 'code_name', 'check_item', 'check_daily_crawler', 'check_min_crawler'],
                                         index=df_stock_all_temp['id'])

        for i in range(len(self.dc.cc.code_df)):
            self.df_stock_all.loc[i, 'code'] = self.dc.cc.code_df.iloc[i][1]
            self.df_stock_all.loc[i, 'code_name'] =  self.open_api.dynamicCall("GetMasterCodeName(QString)", self.dc.cc.code_df.iloc[i][1])
        ...
        for i in range(len(self.dc.cc.code_df_kospi)):
            ...
            self.df_stock_kospi.loc[i, 'code_name'] =  self.open_api.dynamicCall("GetMasterCodeName(QString)", self.dc.cc.code_df_kospi.iloc[i][1])
        ...
        for i in range(len(self.dc.cc.code_df_kosdaq)):
            ...
            self.df_stock_kosdaq.loc[i, 'code_name'] =  self.open_api.dynamicCall("GetMasterCodeName(QString)", self.dc.cc.code_df_kosdaq.iloc[i][1])
        ...
        for i in range(len(self.dc.cc.code_df_konex)):
            ...
            self.df_stock_konex.loc[i, 'code_name'] =  self.open_api.dynamicCall("GetMasterCodeName(QString)", self.dc.cc.code_df_konex.iloc[i][1])
        ...
        for i in range(len(self.dc.cc.code_df_managing)):
            ...
            # code_name은 self.dc.cc.code_df.iloc[i][0]에 있는데 여기에 이름이랑 open_api에 있는 이름이랑 다를 수 있다. 그래서 이런식으로 구현
            self.df_stock_managing.loc[i, 'code_name'] =  self.open_api.dynamicCall("GetMasterCodeName(QString)", self.dc.cc.code_df_managing.iloc[i][1])
        ...
        for i in range(len(self.dc.cc.code_df_insincerity)):
            ...
            # code_name은 self.dc.cc.code_df.iloc[i][0]에 있는데 여기에 이름이랑 open_api에 있는 이름이랑 다를 수 있다. 그래서 이런식으로 구현
            self.df_stock_insincerity.loc[i, 'code_name'] =  self.open_api.dynamicCall("GetMasterCodeName(QString)", self.dc.cc.code_df_insincerity.iloc[i][1])
    ...
    def set_min_crawler_table(self, code, code_name):
            ...
            df_temp = DataFrame(df,
                                columns=['date','check_item','code', 'code_name', 'd1_diff_rate', 'close','open', 'high', 'low',
            ...
            df_temp['d1_diff_rate'] = round((df_temp['close'] - df_temp['close'].shift(1)) / df_temp['close'].shift(1) * 100, 2)
    ...
    def db_to_today_profit_list(self):
        ...
        today_profit_item_temp = {'date': [], 'code': [], 'code_name': [], 'amount': [], 'today_profit': [], 'earning_rate': []}
    ...
    def db_to_jango(self):
        ...
        jango_temp = {'id': [], 'date': [], 'total_asset': [], 'today_profit': [], 'total_profit': [], 'total_invest': [], 'd2_deposit': [],
        ...
        jango_col_list=['date', 'today_earning_rate', 'total_asset', 'today_profit', 'total_profit', 'total_invest',
         ...
         'max_reinvest_count', 'invest_limit_rate', 'invest_unit', 'min_invest_unit', 'max_invest_unit',
         'avg_close_multiply_rate', 'max_reinvest_unit', 'rate_std_sell_point', 'limit_money', 'total_profitcut',
         'total_losscut', 'total_profitcut_count', 'total_losscut_count', 'loan_money', 'start_kospi_point',
         'start_kosdaq_point', 'end_kospi_point', 'end_kosdaq_point', 'today_buy_count', 'today_buy_total_sell_count',
         'today_buy_total_possess_count', 'today_buy_today_profitcut_count', 'today_buy_today_profitcut_rate',
         'today_buy_today_losscut_count', 'today_buy_today_losscut_rate',
         'today_buy_total_profitcut_count', 'today_buy_total_profitcut_rate', 'today_buy_total_losscut_count',
         ...
         'today_buy_reinvest_count5_sell_profitcut_count', 'today_buy_reinvest_count5_sell_losscut_count',
